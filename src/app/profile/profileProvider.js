...
export const showUserMyUnive = async (user_id) => {
    ...
    if(showUserMyUniveResult[0][0]) {
        for (let i = 0; i < showUserMyUniveResult[0].length; i++) {
            ...
            const postInfo = await postIdCheck(showUserMyUniveResult[0][i].post_id);

            // 모임 인원 계산
            const current_people = postInfo[0][0].current_people;
            const limit_people = postInfo[0][0].limit_people;
            showUserMyUniveResult[0][i].people_status = current_people + "/" + limit_people;
...
export const showUserParticipate = async (user_id) => {
    ...
    if(showUserParticipateResult[0][0]) {
        for (let i = 0; i < showUserParticipateResult[0].length; i++) {
            ...
            const postInfo = await postIdCheck(showUserParticipateResult[0][i].post_id);

            // 모임 인원 계산
            const current_people = postInfo[0][0].current_people;
            const limit_people = postInfo[0][0].limit_people;
            showUserParticipateResult[0][i].people_status = current_people + "/" + limit_people;
